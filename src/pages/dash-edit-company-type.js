...
export default function DashCompanyType() {

  const [name, setName ] = useState();
  const [id, setID ] = useState();

  const [err, setErr] = useState(false);
  const {companyTypeId} = useParams()


  useEffect(() => {
    const getCompanyType = async () => {
        let api_url ="https://sartor-server-beta.onrender.com/api/v1/sales-agent/company/type/" + companyTypeId;
        ...
        try {
            ...
            setName(res.data.data.typeOfCompany);
...
const[loading, setLoading] = useState(false)
  const postJobCategory = (e) => {
    ...
    let apiURL = BASEURL + "/sales-agent/company/type/edit" 
    let adminToken = localStorage.getItem("sator-agent-token");
    console.log(adminToken)
    ...
    axios.put(apiURL, companyTypeData, {headers})
    .then((res)=> {
        console.log(res.data)
        alert("Company Type updated")
    }).catch((err)=>{
        console.log(err)
    })
    .finally(()=>{
            ...
            <div className="panel panel-default">
              <div className="panel-heading wt-panel-heading p-a20">
                <h4 className="panel-tittle m-a0">
                  <i className="fa fa-suitcase"></i>                 </h4>
              </div>
                    ...
                    <div className="col-lg-12 col-md-12">
                      <div className="text-left">
                        <button type="submit" className="site-button m-r5">
                         {loading ? "processing" : " Publish Job Category"}
                        </button>
                        {/* <button
                          type="submit"
